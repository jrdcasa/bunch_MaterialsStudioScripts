...
from analysis_xcd.extract import parse_std_files, check_std_file
import utils
import datetime
import os
...
def parse_arguments():
    ...
    # Create the parser
    cli_parser = argparse.ArgumentParser(description='Help to analysisstd program.')
    ...
    group2.add_argument("--check", "-c", help="Give information of the STD file", action='store',
                        default=False, metavar='<STD file name>')
    ...
    group6.add_argument("--template", "-t", help="Template or pattern for the files to analyze (default *.std)", action='store',
                        default=False, metavar='Path')

    args = cli_parser.parse_args()

    if args.usage:
        utils.print_header_std()
        utils.print_usage()
        exit()

    if args.version:
        print('analysisstd version: {} ({})'.format(VERSION, DATE))
        exit()

    if args.check:
        check_std_file(args.check)
        exit()
...
if __name__ == "__main__":
    ...
    parse_std_files(lf, "Solubility parameter", avg_time_properties=avg_time_parameters)
